...
>
> Lo que **no** cambia: cuando el módulo de sincronización exista, la escritura del repartidor entra por `POST /sync/operations` y no por este `PATCH`. La ruta definitiva sigue siendo la de arriba; el `PATCH` queda como camino de oficina —corregir o registrar lo que el chofer no pudo sincronizar— o se restringe a ADMIN/SELLER en ese momento. La decisión de restringirlo se toma junto con el módulo de sync, no antes: hoy es la única forma que tiene un `DRIVER` de registrar una entrega, exactamente como ya lo era para el simple cambio de estado.
