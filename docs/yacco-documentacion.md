...
4. Planificar rutas de reparto y ejecutarlas desde la web, que el repartidor usa en línea desde el celular («Mi ruta»), registrando en cada parada entregas, recogida de envases y cobros.
...
| **Repartidor**            | Ejecuta la ruta: entrega bidones llenos, recoge vacíos y cobra en el punto de entrega.                                                  | Web en el celular («Mi ruta») | Ruta del día ordenada; registro de entrega, recogida y cobro por parada desde el celular, con mínima fricción.                        |
...
- La adopción por parte del repartidor es la condición de éxito operativa: si registrar una parada en la app cuesta más esfuerzo que anotarla en el cuaderno, los datos no se capturarán. La vista del repartidor debe priorizar la velocidad de captura sobre la exhaustividad. Funcionar sin señal quedó fuera del alcance (§1.3, supuesto 12).
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| HU-01 | Como administrador, quiero registrar un lote de producción con fecha, responsable del llenado y cantidades llenadas por tipo de envase, para mantener la trazabilidad de lo producido. | **E1** Dado stock de vacíos registrado, cuando registro un lote con cantidades por tipo, entonces se crea el lote, el stock de llenos aumenta asociado a él y el de vacíos disminuye en la misma cantidad. <br> **E2** Dado que la cantidad a llenar supera los vacíos registrados, cuando guardo el lote, entonces el lote se registra y el sistema advierte, por tipo de envase, cuántos llenó y cuántos vacíos figuraban en la planta; los vacíos en planta quedan en negativo.                                                                                                                                                                                                                                                                                                                                | M         |
...
| HU-25 | Como administrador, quiero contar los vacíos o los llenos de un tipo de envase en la planta, para que el inventario diga lo que hay en el galpón.                                      | **E1** Dado 20 vacíos en planta según el sistema, cuando cuento 14, entonces se registra un ajuste de salida de 6 y el inventario dice 14. <br> **E2** Dado vacíos en planta en negativo (−3), cuando cuento 4, entonces se registra un ajuste de entrada de 7. <br> **E3** Dado 11 llenos en planta repartidos en dos lotes (6 en el más viejo, 5 en el otro), cuando cuento 3, entonces se descuentan 6 del lote más viejo y 2 del otro, con un ajuste por lote. <br> **E4** Dado 3 llenos en planta, cuando cuento 5, entonces el sistema lo rechaza: los llenos que faltan se anotan como lote en Producción. <br> **E5** Dado lo contado igual a lo del sistema, entonces no se registra nada. <br> **E6** Dado un vendedor o un repartidor, cuando intenta contar la planta, entonces el acceso se rechaza. | M         |

> HU-25 se escribió en el cierre final (2026-09-26), con el código: el ajuste por conteo existía solo para los clientes, y la planta no tenía cómo corregir su inventario sin anotar una baja por daño.
...
| ContainerMovementType | FLEET_ENTRY, FILLING, ROUTE_LOAD, LOAN_DELIVERY, EMPTY_PICKUP, FULL_RETURN, EMPTY_UNLOAD, FULL_SALE, DAMAGE_WRITE_OFF, LOSS_WRITE_OFF, OPENING_BALANCE, COUNT_ADJUSTMENT, LOAN_DELIVERY_VOID, EMPTY_PICKUP_VOID, FULL_SALE_VOID | Cada valor define el efecto sobre el parque y sobre el saldo del cliente (p. ej., LOAN_DELIVERY suma al saldo; EMPTY_PICKUP resta; FULL_SALE saca el envase del parque sin tocar el saldo). Los tres `*_VOID` anulan su movimiento: no registran nada que haya pasado en el galpón, registran que algo que se anotó NO pasó, y son tipos propios porque la liquidación agrega por `type` (ver §3.5). |
...
- **Corrección sin editar ni borrar.** Un error operativo en `sales` o `payments` nunca se corrige editando o borrando la fila: se anula con `voided_at`/`voided_by`/`void_reason` y su motivo, y se vuelve a registrar aparte; todo lo que suma dinero (deuda, liquidación, reportes) deja de contar la fila anulada, mientras la original sigue en la tabla con su monto. Las tres columnas de anulación van juntas o ninguna, por un `CHECK` escrito a mano en la migración —Prisma no sabe expresar esa restricción en el schema—, y `route_stops` repite el mismo patrón con `corrected_at`/`corrected_by`/`correction_reason` para la parada que originó esa venta o ese cobro.
...
> **Alcance real:** la parte móvil de S6 y S7 (la app sin conexión y la sincronización idempotente) quedó fuera del alcance (§1.3, supuesto 12): lo que esos sprints entregaron se construyó en la web.
...
> **Estado real, decidido con el dueño de la planta:** que la oficina registre cada parada desde la web, con el chofer dictando o anotando en papel, tiene una consecuencia directa: un error de anotación —una cantidad distinta, un cobro que en realidad no llegó, una parada cargada como fallida cuando sí se entregó— es el caso normal del trabajo diario, no la excepción. CLAUDE.md prohíbe corregirlo editando o borrando el registro, así que existe `PATCH /routes/:id/stops/:stopId/correction` (solo ADMIN, HU-24): anula lo anotado con su motivo y lo vuelve a registrar, en una sola transacción, sin tocar la fila original.
...
| POST           | `/auth/refresh`                        | Renovación del token de acceso; el refresh token viaja en la cookie httpOnly `yacco_refresh` (D-024)                                                                                                                                                                                                   | Con cookie de sesión                   |
| POST           | `/auth/logout`                         | Cierra la sesión del navegador: borra la cookie del refresh                                                                                                                                                                                                                                            | Público                                |
| GET/POST/PATCH | `/users`                               | Gestión de usuarios y sus roles                                                                                                                                                                                                                                                                        | GET: ADMIN, SELLER · POST/PATCH: ADMIN |
...
| GET            | `/routes/:id/truck-stock`              | Por tipo de envase, lo cargado y lo que queda arriba del camión (cargado menos entregado menos vendido, según el libro)                                                                                                                                                                                | ADMIN, SELLER, DRIVER (solo su ruta)   |
| GET            | `/routes/:id`                          | Detalle de la ruta con paradas y carga; cada parada trae su venta y su cobro VIGENTES y el sello de corrección si la hubo — el listado de rutas no trae las ventas                                                                                                                                     | ADMIN, SELLER, DRIVER                  |
| PATCH          | `/routes/:id/stops/:stopId`            | Marca una parada entregada o fallida; DELIVERED registra venta, envases y cobro en una transacción (ver la nota de arriba)                                                                                                                                                                             | ADMIN, SELLER, DRIVER                  |
| PATCH          | `/routes/:id/stops/:stopId/correction` | Corrige una parada ya registrada: anula lo anotado (con motivo obligatorio) y la vuelve a registrar, en una sola transacción; no edita ni borra la venta anterior (HU-24)                                                                                                                              | ADMIN                                  |
| GET            | `/routes/:id/settlement`               | Conciliación de la ruta: lo esperado según el libro y, si ya se liquidó, lo persistido — sirve antes de liquidar; informa además si se corrigió una parada después del cierre                                                                                                                          | ADMIN, SELLER                          |
...
| GET            | `/payments`                            | Bandeja de pagos, con filtros (incluido por estado); excluye abonos de apertura salvo `includeOpeningBalance=true`, y totales sobre el filtro completo; cada fila trae su anulación si la hubo, y un cobro anulado sigue listado y sigue sumando en los totales, que describen el filtro y no un saldo | ADMIN, SELLER                          |
...
| POST           | `/sync/operations`                     | Fuera del alcance (§1.3): aplicación idempotente de operaciones offline                                                                                                                                                                                                                                | DRIVER                                 |
| POST           | `/evidence/presign`                    | Fuera del alcance (§1.3): URL prefirmada para subir una evidencia                                                                                                                                                                                                                                      | DRIVER                                 |
...
| E2E web     | Flujos críticos del administrador: planificar una ruta con carga FIFO y liquidarla                                                                                                                                                                                                                  | Playwright                                                                                 | En PRs que tocan `apps/web-nuxt` |
