...
Reglas del workspace para el agente de código. Este archivo reemplaza a
CLAUDE.md como fuente única; si encontrás CLAUDE.md con contenido propio,
avisá en vez de seguir dos reglas distintas.
...
- Bajá la API antes de cualquier `prisma generate`: con `dev:api` corriendo el
  engine queda tomado y la generación falla a mitad.
...
- Once an operational record is written, a mistake in it is NEVER corrected by
  ...
  correction is an inverse movement, or —for a stop— the void-and-re-register
  of `PATCH /api/v1/routes/:id/stops/:stopId/correction`, ADMIN only, always
  with a reason. La regla no lleva calificativo a propósito: no existe hoy
  ninguna fila "sincronizada", así que decir "nunca editando registros
  sincronizados" no seleccionaría nada y dejaría editable todo lo demás, que
  es todo lo que hay.
- Lo anulado NO desaparece: la fila queda con su monto y sus columnas de
  anulación (`voided_at`/`voided_by`/`void_reason`, las tres juntas o
  ninguna, por un CHECK escrito a mano), todo lo que suma dinero deja de
  contarla, y las lecturas la muestran marcada en vez de esconderla.
...
## Tests — el criterio que manda

- Domain logic is TDD: acceptance criteria in spec §2.4 (Gherkin) become tests
  first.
- **Un test que arma a mano el estado que la feature debería producir no prueba
  la feature.** Por cada cambio de producción: revertilo, corré el test,
  VELO EN ROJO, volvé a aplicarlo. Esa salida en rojo va en el cuerpo del PR.
  Sin ella el test no cuenta como evidencia.
- Si la rama bajo prueba tiene un fallback o una condición previa que ya cubría
  el caso, elegí datos donde no se dispare — si no, estás midiendo lo viejo.
  Ejemplo real: para probar que una lectura filtra lo anulado hacen falta DOS
  filas con valores distintos, una anulada y una viva; con una sola el test
  pasa aunque el filtro no exista.
- Las afirmaciones de la web van por texto visible y rol accesible, nunca por
  `className`.

## Calidad — no negociable

SonarCloud: 80% de cobertura en código nuevo, 3% de duplicación. NUNCA bajes un
umbral ni agregues una exclusión para pasar el gate. Si dispara duplicación,
extraé lo compartido. Si dispara cobertura, escribí el test que falta.
...
- Ante una ambigüedad de dominio —qué debería pasar, no cómo implementarlo—
  parás y preguntás. No la resuelvas eligiendo la opción más fácil de
  programar.

## Con Antigravity no corren los hooks

- Formateo: después de editar, correr a mano `pnpm exec prettier --write` sobre el archivo, más `pnpm exec eslint --fix` si es .ts/.tsx/.js/.jsx/.cjs/.mjs. Es exactamente lo que hace el hook de Claude Code, y refleja lo que `lint-staged` declara en package.json.
...
- .agents/skills/ contiene el contenido de las skills; .claude/skills/ contiene los punteros con su frontmatter.
- .claude/agents/ contiene los subagentes. Nota: No se portaron a .agents/agents/ porque la documentación de Antigravity no documenta una ruta declarativa para subagentes, por lo tanto, escribirlos ahí sería inventar una ruta que fallaría silenciosamente.
