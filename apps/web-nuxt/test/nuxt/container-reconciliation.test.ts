import { registerEndpoint, renderSuspended } from "@nuxt/test-utils/runtime";
import { screen, within } from "@testing-library/vue";
import userEvent from "@testing-library/user-event";
import type { ContainerReconciliation, ContainerReconciliationDiscrepancy } from "@yacco/shared";
import { afterEach, beforeEach, describe, expect, it } from "vitest";
import App from "~/app.vue";
import { failWith } from "../support/route-detail";
import { resetSession, signIn } from "../support/session";

const cleanups: Array<() => void> = [];
const LOCATION_ID = "11111111-1111-4111-8111-111111111111";
const CONTAINER_TYPE_ID = "22222222-2222-4222-8222-222222222222";

function discrepancy(
  overrides: Partial<ContainerReconciliationDiscrepancy> = {},
): ContainerReconciliationDiscrepancy {
  const ledgerQuantity = overrides.ledgerQuantity ?? 8;
  const materializedQuantity = overrides.materializedQuantity ?? 5;
  return {
    locationId: LOCATION_ID,
    locationName: "Bodega Los Jazmines",
    containerTypeId: CONTAINER_TYPE_ID,
    containerTypeName: "Bidón 20L",
    ledgerQuantity,
    materializedQuantity,
    difference: ledgerQuantity - materializedQuantity,
    ...overrides,
  };
}

function stubCheck(discrepancies: ContainerReconciliationDiscrepancy[]): { calls: number } {
  const counter = { calls: 0 };
  cleanups.push(
    registerEndpoint("/api/v1/container-reconciliation", () => {
      counter.calls++;
      const body: ContainerReconciliation = {
        checkedAt: "2026-08-28T15:30:00.000Z",
        discrepancyCount: discrepancies.length,
        discrepancies,
      };
      return body;
    }),
  );
  return counter;
}

async function renderPage(roles: ("ADMIN" | "SELLER")[] = ["ADMIN"]) {
  cleanups.push(signIn(roles));
  await renderSuspended(App, { route: "/container-reconciliation" });
  await screen.findByRole("heading", { name: "Cuadre de envases", level: 1 });
}

async function rowOf(text: string): Promise<HTMLElement> {
  return (await screen.findByText(text)).closest("tr") as HTMLElement;
}

describe("Cuadre de envases", () => {
  beforeEach(async () => {
    resetSession();
    await navigateTo("/login");
  });

  afterEach(() => {
    for (const cleanup of cleanups.splice(0)) cleanup();
  });

  it("explica que son dos cuentas independientes y que informa sin corregir", async () => {
    stubCheck([]);

    await renderPage();

    expect(await screen.findByRole("heading", { name: "Qué se compara" })).toBeTruthy();
    expect(screen.getByText("Lo que pasó")).toBeTruthy();
    expect(screen.getByText("Lo que el sistema muestra")).toBeTruthy();
    expect(screen.getByText(/informa y no corrige/)).toBeTruthy();
  });

  it("sin descuadres lo dice como buena noticia, no como lista vacía", async () => {
    stubCheck([]);

    await renderPage();

    expect(await screen.findByText("Las dos cuentas coinciden")).toBeTruthy();
    expect(screen.getByText(/El saldo que muestran las pantallas es confiable/)).toBeTruthy();
    expect(screen.queryByRole("table")).toBeNull();
  });

  it("muestra la fecha en que se corrió el cuadre", async () => {
    stubCheck([]);

    await renderPage();

    expect(await screen.findByText("Revisado el 28/08/2026 10:30.")).toBeTruthy();
  });

  it("lista un descuadre con las dos cuentas y el sentido de la diferencia", async () => {
    stubCheck([discrepancy({ ledgerQuantity: 8, materializedQuantity: 5 })]);

    await renderPage();

    expect(
      await screen.findByText("Hay 1 saldo que no coincide con sus movimientos."),
    ).toBeTruthy();
    const row = await rowOf("Bodega Los Jazmines");
    expect(within(row).getByText("Bidón 20L")).toBeTruthy();
    expect(within(row).getByText("8")).toBeTruthy();
    expect(within(row).getByText("5")).toBeTruthy();
    expect(within(row).getByText("+3")).toBeTruthy();
    expect(within(row).getByText("Al saldo le faltan 3 envases")).toBeTruthy();
  });

  it("un saldo con envases de más se describe al revés, no en valor absoluto", async () => {
    stubCheck([discrepancy({ ledgerQuantity: 2, materializedQuantity: 4 })]);

    await renderPage();

    const row = await rowOf("Bodega Los Jazmines");
    expect(within(row).getByText("-2")).toBeTruthy();
    expect(within(row).getByText("El saldo tiene 2 envases de más")).toBeTruthy();
  });

  it("con un solo envase de diferencia habla en singular", async () => {
    stubCheck([discrepancy({ ledgerQuantity: 4, materializedQuantity: 3 })]);

    await renderPage();

    expect(await screen.findByText("Al saldo le falta 1 envase")).toBeTruthy();
  });

  it("varios descuadres se cuentan en plural", async () => {
    stubCheck([
      discrepancy(),
      discrepancy({ locationId: "loc-2", locationName: "Farmacia San Judas" }),
    ]);

    await renderPage();

    expect(
      await screen.findByText("Hay 2 saldos que no coinciden con sus movimientos."),
    ).toBeTruthy();
  });

  it("una ubicación que no resuelve se nombra como tal, con su id a la vista", async () => {
    stubCheck([discrepancy({ locationName: null })]);

    await renderPage();

    const row = await rowOf("Ubicación desconocida");
    expect(within(row).getByText(LOCATION_ID)).toBeTruthy();
  });

  it("un movimiento sin ubicación lo dice en vez de mostrar una celda vacía", async () => {
    stubCheck([discrepancy({ locationId: null, locationName: null })]);

    await renderPage();

    expect(await screen.findByText("Ubicación desconocida")).toBeTruthy();
    expect(screen.getByText("el movimiento no indica ubicación")).toBeTruthy();
  });

  it("un tipo de envase que no resuelve también se nombra", async () => {
    stubCheck([discrepancy({ containerTypeName: null })]);

    await renderPage();

    const row = await rowOf("Tipo de envase desconocido");
    expect(within(row).getByText(CONTAINER_TYPE_ID)).toBeTruthy();
  });

  it("«Volver a revisar» vuelve a correr el cuadre", async () => {
    const counter = stubCheck([]);

    await renderPage();
    await screen.findByText("Las dos cuentas coinciden");
    expect(counter.calls).toBe(1);

    await userEvent.setup().click(screen.getByRole("button", { name: "Volver a revisar" }));

    expect(await screen.findByText("Las dos cuentas coinciden")).toBeTruthy();
    expect(counter.calls).toBe(2);
  });

  it("muestra el error y permite reintentar", async () => {
    let attempt = 0;
    cleanups.push(
      registerEndpoint("/api/v1/container-reconciliation", (event) => {
        attempt++;
        if (attempt === 1) return failWith(500, "Base de datos no disponible")(event);
        const body: ContainerReconciliation = {
          checkedAt: "2026-08-28T15:30:00.000Z",
          discrepancyCount: 0,
          discrepancies: [],
        };
        return body;
      }),
    );

    await renderPage();

    expect((await screen.findByRole("alert")).textContent).toContain("Base de datos no disponible");

    await userEvent.setup().click(screen.getByRole("button", { name: "Reintentar" }));

    expect(await screen.findByText("Las dos cuentas coinciden")).toBeTruthy();
  });

  it("traduce el 403 del backend", async () => {
    cleanups.push(
      registerEndpoint("/api/v1/container-reconciliation", failWith(403, "Forbidden resource")),
    );

    await renderPage();

    expect((await screen.findByRole("alert")).textContent).toContain(
      "Solo un administrador puede correr el cuadre de envases",
    );
  });

  it("un vendedor ve por qué no puede correrlo, y no se llama a la API", async () => {
    const counter = stubCheck([]);

    await renderPage(["SELLER"]);

    expect(await screen.findByText("Este cuadre es solo para administradores")).toBeTruthy();
    expect(counter.calls).toBe(0);
    expect(screen.queryByRole("button", { name: "Volver a revisar" })).toBeNull();
  });
});
